...
const BASE_COLUMNS: ColumnDef[] = [
  ...
  },
  {
    ...
    type: "number",
    example: 100,
    notes: "Cost price per unit. Used for margin calculation.",
    aliases: ["purchase price", "cost price", "buying price", "cost", "purchase_price", "cp"],
  },
  {
    key: "sellingPrice",
...
export function getImportTemplate(businessType: string): ImportTemplate {
  ...
  const firstExample: Record<string, string | number> = {
    ...
    "Purchase Price (₹)": seed?.purchasePrice ?? 100,
  ...
  const secondExample: Record<string, string | number> = {
    ...
    "Purchase Price (₹)": secondSeed?.purchasePrice ?? 200,
