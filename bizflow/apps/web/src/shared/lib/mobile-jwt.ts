...
// A-7 FIX: Warn when dedicated mobile secrets are not set
if (!process.env.MOBILE_JWT_SECRET) {
  console.warn('[MobileJWT] ⚠ MOBILE_JWT_SECRET not set — falling back to NEXTAUTH_SECRET. Set a dedicated secret for production.');
}
if (!process.env.MOBILE_REFRESH_SECRET) {
  console.warn('[MobileJWT] ⚠ MOBILE_REFRESH_SECRET not set — falling back to NEXTAUTH_SECRET. Set a dedicated secret for production.');
}
