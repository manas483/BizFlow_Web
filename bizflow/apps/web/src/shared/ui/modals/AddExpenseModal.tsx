...
export default function AddExpenseModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (open && form.invoiceNumbers.length > 0) {
      ...
      fetch(`/api/inventory/products/purchase-invoices?invoices=${form.invoiceNumbers.join(",")}`)
        .then(res => res.json())
        .then(data => {
  ...
  const getPreviewDetails = () => {
    ...
    const totalUnits = previewProducts.reduce((sum, p) => {
      if (form.excludedProductIds.includes(p.id)) return sum;
      const stock = p.stock || 0;
      const unitsPerBag = p.unitsPerBag || 1;
      return sum + (stock / unitsPerBag);
    }, 0);

    if (totalUnits <= 0) return previewProducts.map(p => ({
      id: p.id,
      name: p.name,
      sku: p.sku,
      ...
      newAdditional: p.transportCost,
      newLanded: p.purchasePrice,
      change: 0,
      excluded: true
    }));
    ...
    return previewProducts.map(p => {
      const excluded = form.excludedProductIds.includes(p.id);
      const unitsPerBag = p.unitsPerBag || 1;
      const additionalCostFromThis = excluded ? 0 : (expensePerTransportUnit / unitsPerBag);
                  ...
                  <table className="w-full text-left text-[11px]">
                    ...
                    <tbody className="divide-y divide-primary/5">
                      {previewDetails.map((item) => (
                        <tr key={item.id} className={`text-primary/70 hover:text-primary ${item.excluded ? 'opacity-40' : ''}`}>
