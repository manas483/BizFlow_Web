...
export const QuotationDocument = ({ quotation }: { quotation: any }) => {
  const gstInclusive: boolean = quotation.business?.gstInclusive ?? false;

  const isInterState = quotation.placeOfSupply && quotation.business?.state
    ? quotation.placeOfSupply.toLowerCase() !== quotation.business.state?.toLowerCase()
    : false;

  const quotationDate = new Date(quotation.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' }).replace(/ /g, '-');
  ...
  quotation.items.forEach((item: any) => {
    ...
    // Round to 2 decimals for perfect "on-paper" math
  ...
  const bizState = quotation.business?.state || '';
  const custState = quotation.customer?.state || '';
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.container}>
          ...
          <View style={s.header}>
            ...
            <View style={s.headerRight}>
              <Text style={[s.label, s.bold]}>Estimate</Text>
            </View>
          </View>
          ...
          <View style={s.topGrid}>
            <View style={s.colLeft}>
              <View style={s.section}>
                <Text style={s.value}>{quotation.business?.name}</Text>
                {quotation.business?.address && <Text style={s.text}>{quotation.business.address}</Text>}
                {quotation.business?.phone && <Text style={s.text}>{quotation.business.phone}</Text>}
                <Text style={s.text}>GSTIN: {quotation.business?.gstNumber || 'Unregistered'}</Text>
              ...
              <View style={s.sectionNoBorder}>
                ...
                <Text style={[s.value, { marginTop: 2 }]}>{quotation.customer?.name || 'Cash Customer'}</Text>
                {quotation.customer?.address && <Text style={s.text}>{quotation.customer.address}</Text>}
                {quotation.customer?.gstNumber && <Text style={s.text}>GSTIN      : {quotation.customer.gstNumber}</Text>}
            ...
            <View style={s.colRight}>
              <View style={s.rRow}>
                <View style={s.rCol}>
                  <Text style={s.label}>Quotation No.</Text>
                  <Text style={s.value}>{quotation.quotationNo}</Text>
                </View>
                <View style={s.rColLast}>
                  <Text style={s.label}>Quotation Date</Text>
                  <Text style={s.value}>{quotationDate}</Text>
                </View>
              </View>
          ...
          <View>
            ...
            {Object.entries(taxMap).map(([hsn, v]) => {
              const rate = quotation.items.find((i: any) => i.hsnCode === hsn || String(i.gstRate) === hsn)?.gstRate || 0;
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <View style={{ width: '50%', padding: 4, borderRightWidth: 1, borderColor: BORDER_COLOR }}>
              <Text style={[s.bold, { marginBottom: 4 }]}>Bank Details</Text>
              <View style={{ flexDirection: 'row', marginBottom: 2 }}>
                <Text style={{ width: 60, fontSize: 8 }}>Account Name:</Text>
                <Text style={{ fontSize: 8, flex: 1 }}>{quotation.business?.bankName || quotation.business?.name || '—'}</Text>
              </View>
              <View style={{ flexDirection: 'row', marginBottom: 2 }}>
                <Text style={{ width: 60, fontSize: 8 }}>IFSC Code:</Text>
                <Text style={{ fontSize: 8, flex: 1 }}>{quotation.business?.ifscCode || '—'}</Text>
              </View>
              <View style={{ flexDirection: 'row', marginBottom: 2 }}>
                <Text style={{ width: 60, fontSize: 8 }}>Account No:</Text>
                <Text style={{ fontSize: 8, flex: 1 }}>{quotation.business?.accountNumber || '—'}</Text>
              </View>
              <View style={{ flexDirection: 'row', marginBottom: 2 }}>
                <Text style={{ width: 60, fontSize: 8 }}>Bank & Branch:</Text>
                <Text style={{ fontSize: 8, flex: 1 }}>{quotation.business?.branch || '—'}</Text>
              </View>
            </View>

            {/* Column 2: Terms & Conditions */}
            <View style={{ width: '50%', padding: 4, borderColor: BORDER_COLOR }}>
              <Text style={[s.bold, { marginBottom: 4 }]}>Terms and Conditions</Text>
              <Text style={{ fontSize: 8, marginBottom: 2 }}>1. This is a quotation, not an invoice.</Text>
              <Text style={{ fontSize: 8 }}>2. Prices are subject to change after the valid until date.</Text>
            </View>
        ...
        <Text style={s.bottomNote} fixed>This is a Computer Generated Document</Text>
      </Page>
    </Document>
  );
};
