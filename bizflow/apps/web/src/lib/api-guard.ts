...
async function sessionFromBearer(bearerToken: string): Promise<AuthSession | null> {
  ...
  // Refresh live permissions + role from DB on every authenticated request.
  // Same pattern as the NextAuth JWT callback — ensures permission changes
  // propagate to mobile clients within one token TTL (15 min).
  try {
