...
import { ShoppingCart, Plus, Trash2, ChevronDown, AlertTriangle } from "lucide-react";
...
export default function NewSaleModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to create sale. Check stock availability.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={handleClose}
      title="Create GST Invoice" subtitle="GST-compliant tax invoice for goods or services"
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Invoice Date">
            <ModalInput type="date" defaultValue={new Date().toISOString().split("T")[0]} />
          </FormField>
        </div>
        ...
        <ModalFooter onClose={handleClose} loading={loading} submitLabel="Generate GST Invoice" />
      </form>
