...
import {
  ...
  CalendarCheck, Calculator, Landmark,
} from "lucide-react";
import { cn, getInitials } from "@/lib/utils";
import { useApp } from "@/context/AppContext";
import ConfirmDialog from "@/components/ui/ConfirmDialog";

const navItems: { href: string; icon: any; label: string; permission: Permission; adminOnly?: boolean }[] = [
      ...
      >
        ...
        <nav className="flex-1 py-4 px-2 space-y-0.5 overflow-y-auto scrollbar-hide">
          {mounted && navItems.filter(item => {
            ...
            // adminOnly=true: only show to SUPER_ADMIN
            if (item.adminOnly === true && role !== 'SUPER_ADMIN') return false;
